...
namespace tigerball.Tests
{
    public abstract class tigerballTestBase : AbpIntegratedTestBase<tigerballTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected tigerballTestBase()
        {
        ...
        protected void UsingDbContext(Action<tigerballDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<tigerballDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<tigerballDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<tigerballDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<tigerballDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<tigerballDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<tigerballDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<tigerballDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<tigerballDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<tigerballDbContext, Task<T>> func)
        {
            T result;
